...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        paths = np.random.choice(self.folders, self.num_classes).tolist()
        lbls = np.eye(self.num_classes).tolist()

        images = []
        labels = []
        for path, lbl in zip(paths, lbls): 
            labels_and_images = get_images([path], [lbl], nb_samples=self.num_samples_per_class)
        
            labels.append([x[0] for x in labels_and_images])
            images.append([self.image_file_to_array(x[1], 784) for x in labels_and_images])
        ...
        return images.type('torch.FloatTensor'), labels.type('torch.FloatTensor')
        #############################
