...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        in_images = input_images.view(B, N * K_plus_1, -1)

        # Set the label for the last N examples to be 0
        in_labels = torch.clone(input_labels)
        in_labels[:, K_plus_1-1, :, :] = 0
        in_labels = in_labels.view(B, N * K_plus_1, N)

        # Concatenate input_images and input_labels along the feature dimension
        input_tensor = torch.cat([in_images, in_labels], dim=-1)

        # Pass data through the first layer
        output1, _ = self.layer1(input_tensor)
        ...
        output = output2.view(B, K_plus_1, N, N)
