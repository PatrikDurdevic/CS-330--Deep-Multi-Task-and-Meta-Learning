...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # accuracy_query_batch.
            images_support_prototypes = self._network(images_support)

            images_query_prototypes = self._network(images_query)

            prototypes = {}
            for prototype in torch.unique(labels_support):
                prototypes[prototype.item()] = torch.mean(images_support_prototypes[labels_support == prototype])

            labels_support_hat = torch.zeros(images_support_prototypes.shape[0], torch.unique(labels_support).shape[0])
            for id_query, image_support_prototype in enumerate(images_support_prototypes):
                for id_label, label in enumerate(torch.unique(labels_support)):
                    normalization_term = torch.tensor(0.0)
                    for prototype in prototypes.values():
                        normalization_term += torch.exp(-torch.norm(prototype - image_support_prototype))
                    labels_support_hat[id_query, id_label] = torch.exp(-torch.norm(prototypes[label.item()] - image_support_prototype)) / normalization_term
            
            labels_query_hat = torch.zeros(images_query_prototypes.shape[0], torch.unique(labels_support).shape[0])
            for id_query, image_query_prototype in enumerate(images_query_prototypes):
                for id_label, label in enumerate(torch.unique(labels_support)):
                    normalization_term = torch.tensor(0.0)
                    for prototype in prototypes.values():
                        normalization_term += torch.exp(-torch.norm(prototype - image_query_prototype))
                    labels_query_hat[id_query, id_label] = torch.exp(-torch.norm(prototypes[label.item()] - image_query_prototype)) / normalization_term

            loss_batch.append(F.cross_entropy(labels_query_hat, labels_query))
            accuracy_support_batch.append(util.score(labels_support_hat, labels_support))
            accuracy_query_batch.append(util.score(labels_query_hat, labels_query))
            # ********************************************************
            # ******************* YOUR CODE HERE *********************
            # ********************************************************
        return (
